...
def sample_base_color(category):
    if category == "red":
        return np.array([random.choice([random.randint(0, 60), random.randint(300, 360)]), 50, random.randint(0, 100)])
    elif category == "green":
        return np.array([random.randint(60, 180), 50, random.randint(0, 100)])
    elif category == "blue":
        return np.array([random.randint(180, 300), 50, random.randint(0, 100)])
...
def generate_trial(condition, theta, epsilon):
    ...
    if condition == "close":
        ...
        while len(colors) < 2:
            new_color_hls = np.array([random.randint(0, 360), 50, random.randint(0, 100)])
...
def generate_trials(num_trials, theta, epsilon):
    ...
    for condition in trials.keys():

        generated = 0
        used_targets = set()  # <--- 新增：记录已用的 target LAB
        while generated < num_trials:
            trial = generate_trial(condition, theta, epsilon)
            if trial is None:
                continue

            flattened_trial = trial.reshape(-1, 3)
            target_color = tuple(round(x, 1) for x in flattened_trial[0])  # <--- 当前 target

            # <--- 新增：检查是否重复
            if target_color in used_targets:
                continue  # 跳过重复 target
            used_targets.add(target_color)
            ...
            if condition == "close":
                if not (epsilon <= d1_dist < theta and epsilon <= d2_dist < theta):
                    continue  # Skip misclassified
            elif condition == "far":
                if not (d1_dist > theta and d2_dist > theta):
                    continue
            elif condition == "split":
            ...
            # Passed checks — continue processing
            shuffled = flattened_trial.tolist()
            random.shuffle(shuffled)


            target_color = tuple(round(x, 1) for x in flattened_trial[0])
            shuffled_colors = [tuple(round(x, 1) for x in color) for color in shuffled]
            position = shuffled_colors.index(target_color)
            color_name = random.choice(color_names)

            trials[condition].append((
                f"\"[{', '.join(map(str, [round(x, 1) for x in flattened_trial[0]]))}]\"",
                f"\"[{', '.join(map(str, [round(x, 1) for x in flattened_trial[1]]))}]\"",
                f"\"[{', '.join(map(str, [round(x, 1) for x in flattened_trial[2]]))}]\"",
                f"\"[{', '.join(map(str, [round(x, 1) for x in shuffled[0]]))}]\"",
                f"\"[{', '.join(map(str, [round(x, 1) for x in shuffled[1]]))}]\"",
                f"\"[{', '.join(map(str, [round(x, 1) for x in shuffled[2]]))}]\"",
                position,
                color_name
            ))
            generated += 1
...
def visualize_and_save_trials(trials, max_trials=100):
    for condition, condition_trials in trials.items():
        ...
        for trial_idx, trial in enumerate(condition_trials):
            colors_cielab = [
                tuple(map(float, color.strip('"[]').split(", "))) for color in trial[:6]
            ]
