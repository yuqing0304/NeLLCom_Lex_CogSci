...
# Function to output trials to separate text files in ./data_gray folder
def output_trials_to_files(trials):
    # Create folder if it doesn't exist
    os.makedirs("./data_gray", exist_ok=True)

    for condition, condition_trials in trials.items():
        # Open a new file for each condition inside ./data_gray
        file_path = os.path.join("./data_gray", f"sample_{condition}.txt")
...
def visualize_and_save_trials(trials, max_trials=100):
    for condition, condition_trials in trials.items():
        ...
        plt.tight_layout()
        plt.savefig(f"./data_gray/trials_{condition}.png")
        plt.close()
