...
if __name__ == "__main__":
    ...
    # ===== NEW: Process output_TRAINinitial.txt from all seed folders =====
    base_dir = os.path.dirname(args.input_files[0])
    train_initial_files = []

    # Traverse subdirectories for TRAINinitial files
    for root, dirs, files in os.walk(base_dir):
        for file in files:
            if file == "output_TRAINinitial.txt":
                train_initial_files.append(os.path.join(root, file))

    if train_initial_files:
        print(f"Found {len(train_initial_files)} TRAINinitial files:")
        for f in train_initial_files:
            print(f"  - {f}")
            process_file(f, args.rl_or_sl, args.use_label)
    else:
        print("No output_TRAINinitial.txt files found.")
    # ===== NEW: Process output_TRAINinitial.txt from all seed folders =====
