...
class UniformDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # sender_input = [torch.tensor(self.sender_input[idx][i]).float() for i in range(len(self.sender_input[idx]))]
        sender_input = torch.stack([torch.tensor(self.sender_input[idx][i]).float() for i in range(len(self.sender_input[idx]))])
        # receiver_input = [torch.tensor(self.receiver_input[idx][i]).float() for i in range(len(self.receiver_input[idx]))]
