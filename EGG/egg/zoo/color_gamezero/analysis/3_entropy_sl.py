...
### python entropy_sl.py ../condition3_generated/dump_context/msg_spk_seed111
### python entropy_sl.py ../condition3_generated/dump_context/msg_spk_seed222
### should be the same as the below
...
def plot_entropy_over_epochs(seeds_directories):
    ...
    # Get epoch color names and label color counts
    ...
    epochs = sorted(epoch_color_names.keys(), key=lambda x: int(x))
    ...
    # Use label color entropy as horizontal reference line
    plt.axhline(y=label_color_entropy, color='red', linestyle='--', linewidth=1, label='Entropy of Label Colors')

    # Add text annotation near the line
    plt.text(
        x=25 * 0.95,  # Adjust based on your x-axis range
    ...
    plt.gca().spines[['left', 'bottom']].set_linewidth(0.8)
    ...
    print(f"Label Color Entropy: {label_color_entropy:.4f}")
...
def process_file_freq_spk(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            data.append([hls_str, color_name, label_color])
    # print(f"data length, {len(data)}")
