...
# format: [[271, 50, 25], [225, 50, 77], [210, 50, 23]] -> purple (label=purple)
# in which purple is the ground-truth label
...
def process_file_info_system(file_path, word_info_path):
    ...
    with open(file_path, 'r') as f:
        ...
        for line in f:
            ...
            # Extract the color name part
            color_part = parts[1].strip()
            # print(f"color_part, {color_part}")

            # Updated regex to handle "label=" inside parentheses
            match = re.match(r"(\w+)\s*\(label=(.*?)\)", color_part)

            if match:
                color_name = match.group(1)  # Extract main color
                label_colors = match.group(2)  # Extract label color inside parentheses
            else:
                color_name = color_part  # If no parentheses, just take the color name
                label_colors = None
