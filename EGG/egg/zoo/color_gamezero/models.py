...
class NewReceiver(nn.Module):
    def __init__(self, embed_dim, hidden_dim, num_classes, dropout, sender=None):
        super(NewReceiver, self).__init__()
        self.emb_layer = nn.Embedding(num_classes, hidden_dim)
        if sender is not None:
            self.emb_layer.weight = sender.classifier.weight
            self.emb_layer.weight.requires_grad = False       # freeze for SL
...
class NewSender(nn.Module):
    ...
    def get_embeddings(self):
        return self.classifier.weight.detach().cpu().numpy()
    
    def save_embeddings(self, filename="spkembeddings.npy", out_dir="outputs_emb"):
        os.makedirs(out_dir, exist_ok=True)
        filepath = os.path.join(out_dir, filename)
        np.save(filepath, self.get_embeddings())
