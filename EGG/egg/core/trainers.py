...
class Trainer:
    ...
    def save_receiver_embeddings(self, filename, out_dir):
        return self.game.receiver.agent.save_embeddings(filename, out_dir)

    def save_sender_embeddings(self, filename, out_dir):
        return self.game.sender.agent.save_embeddings(filename, out_dir)

    def get_receiver(self):
        return self.game.receiver.agent
    
    def get_sender(self):
        return self.game.sender.agent
    ...
    def train(self, n_epochs, id_to_color, data_type, test_color, current_seed, if_context):
        ...
        with log_file.open("w") as log_f:
            ...
            for epoch in range(self.start_epoch, n_epochs):
                ...
                if (
                    ...
                    # # ====== (2) EVAL ON TRAINING SET ======
                    # train_loss_dump, train_interaction_dump = self.eval(
                    #     data_type,
                    #     data=self.train_data,
                    #     args_print=(f"TRAIN{epoch}", id_to_color),  # <-- prefix “TRAIN”
                    #     seed=current_seed,
                    #     if_context=if_context,
                    #     log_f=log_f,
                    #     epoch=epoch,
                    ...
                    # # ====== (2) EVAL ON TRAINING SET ======
                    # ===================== DUMP TRAIN OUTPUT AT FINAL EPOCH =====================
                    if epoch == 0:  # Only dump at the last epoch
                        print(f"# Dumping training outputs for the final epoch {epoch + 1}")
                        train_loss_dump, train_interaction_dump = self.eval(
                            data_type,
                            test_color,
                            data=self.train_data,
                            args_print=("TRAINinitial", id_to_color),  # file name: TRAINoutput_final.txt
                            seed=current_seed,
                            if_context=if_context,
                            log_f=log_f,
                            epoch=epoch,
    ...
    def eval(self, data_type, test_color, data=None, args_print=None, seed=None, if_context=False, log_f=None, epoch=None):
        mean_loss = 0.0
        interactions = []
        n_batches = 0
            ...
            with torch.no_grad():
                for batch in validation_data:
                    ...
                    for i in range(interaction.sender_input.size(0)):
                        ...
                        f.write(
                            f"{[[round(x, 1) for x in sublist] for sublist in interaction.sender_input[i].tolist()]} -> "
                            f"{args_print[1][interaction.message[i].item()]} ({args_print[1][interaction.aux_input['color'][i].item()]}) -> "
                            f"{[[round(x, 1) for x in sublist] for sublist in interaction.receiver_input[i].tolist()]} -> "
                            f"{interaction.receiver_output[i].item()} -> "
                            f"{interaction.labels[i].item()} -> "
                            f"{c_name}\n"
                        )
